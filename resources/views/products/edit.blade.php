                ...
                <div class="panel-body">
                    <form method="post" action="{{ route('products.update', $product->id) }}" class="form-horizontal">
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-sm-2">
                                <div class="checkbox {{ $errors->has('is_service') ? 'has-error' : '' }}">
                                    <label>
                                        <input type="checkbox" name="is_service" value="1" v-model="is_service" @if($product->is_service) checked @endif> Is Service Item?
                            ...
                            <div class="col-sm-5">
                                <select id="product-item-id" name="product_item_id" class="form-control" v-bind:required="is_bulk_container">
                                    ...
                                    @foreach($products as $productItem)
                                        <option value="{{ $productItem->id }}" @if((old('product_item_id') ?: $product->product_item_id) == $productItem->id) selected @endif>{{ $productItem->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group {{ $errors->has('product_item_quantity') ? 'has-error' : '' }}" v-bind:class="{hidden: !is_bulk_container}">
                            ...
                            <div class="col-sm-5">
                                <input type="text" class="form-control" id="product-item-quantity" name="product_item_quantity" placeholder="Eg: 1" value="{{ old('product_item_quantity') ?: $product->product_item_quantity }}" v-bind:required="is_bulk_container" />
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-2">
                                <a href="{{ route('products.show', $product->id) }}" class="btn btn-danger btn-block">Cancel</a>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script type="text/javascript">
        const app = new Vue({
            ...
            data: {
                ...
                is_bulk_container: {{ old('is_bulk_container', $product->isBulkContainer()) ? 'true' : 'false' }},
        },
        methods: {
