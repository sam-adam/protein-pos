                ...
                <div class="panel-body">
                    <form method="post" action="{{ route('categories.update', $category->id) }}" class="form-horizontal">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <div class="col-sm-5">
                                <input type="text" id="name" name="name" class="form-control" placeholder="Eg: John Doe" required value="{{ old('name') ?: $category->name }}" />
                            ...
                            <div class="col-sm-5">
                                <select class="form-control" id="parent_id" name="parent_id">
                                    ...
                                    @foreach($roots as $root)
                                        <option value="{{ $root->id }}" @if((old('parent_id') ?: $category->parent_id) == $root->id) selected @endif>{{ $root->name }}</option>
                                    @endforeach
                                </select>
